...
public class TDBlockStates extends BlockStateProvider {
    ...
    private void registerDoor(Supplier<? extends Block> blockSupplier, String texturePath, String modNamespace) {
        ...
        ModelFile br  = models().withExistingParent(baseName + "_bottom_right",      mcLoc("block/door_bottom_right"))     .texture("bottom", bottom).texture("top", top);
        ModelFile bro = models().withExistingParent(baseName + "_bottom_right_open", mcLoc("block/door_bottom_right_open")).texture("bottom", bottom).texture("top", top);
        ModelFile tl  = models().withExistingParent(baseName + "_top_left",          mcLoc("block/door_top_left"))         .texture("bottom", bottom).texture("top", top);
        ModelFile tlo = models().withExistingParent(baseName + "_top_left_open",     mcLoc("block/door_top_left_open"))    .texture("bottom", bottom).texture("top", top);
        ModelFile tr  = models().withExistingParent(baseName + "_top_right",         mcLoc("block/door_top_right"))        .texture("bottom", bottom).texture("top", top);
        ModelFile tro = models().withExistingParent(baseName + "_top_right_open",    mcLoc("block/door_top_right_open"))   .texture("bottom", bottom).texture("top", top);
    ...
    private void slidingDoorBlock(SlidingDoorBlock block,
                                  ModelFile bl, ModelFile blo, ModelFile blto,
                                  ModelFile br, ModelFile bro, ModelFile brto,
                                  ModelFile tl, ModelFile tlo, ModelFile tlto,
                                  ModelFile tr, ModelFile tro, ModelFile trto) {
        getVariantBuilder(block).forAllStates(state -> {
            ...
            ModelFile model = half == DoubleBlockHalf.LOWER
                    ? (hinge == DoorHingeSide.LEFT ? (open ? (inWall ? blo : blto) : bl) : (open ? (inWall ? bro : brto) : br))
                    : (hinge == DoorHingeSide.LEFT ? (open ? (inWall ? tlo : tlto) : tl) : (open ? (inWall ? tro : trto) : tr));
    ...
    private void petDoorBlock(PetDoorBlock block,
                              ModelFile bottom, ModelFile bottomOpen,
                              ModelFile top,    ModelFile topOpen) {
        getVariantBuilder(block).forAllStatesExcept(state -> {
            ...
            ModelFile model = half == Half.BOTTOM ? (open ? bottomOpen : bottom) : (open ? topOpen : top);
