...
public final class ShapeMapProvider implements DataProvider {
    ...
    private static void buildFamily(WoodFamily family, String registryNs, JsonObject add) {
        ...
        for (DoorVariant v : order) {
            if (v == DoorVariant.DISCRETE  && hasDiscrete)  continue; // is the anchor itself
            if (v == DoorVariant.ORIGINAL  && !hasDiscrete) continue; // is the anchor itself
            if (v == DoorVariant.PET)                        continue; // goes in the trapdoor group
            if (v == DoorVariant.TRAPDOOR)                   continue; // sentinel, not an item
