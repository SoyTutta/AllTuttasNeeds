...
public final class TBRecipes {
    ...
    public static void register(RecipeOutput output) {
        RecipeOutput moduleOutput = output.withConditions(ModuleEnabledCondition.BEDS);
        registerFrame(moduleOutput);
        registerBedDyeing(moduleOutput);
        registerBedBlanketing(moduleOutput);

        BedCompatRegistry.loaded().forEach(compat -> compat.families().forEach(family -> {
            Mods owner = compat.mod();
            RecipeOutput familyOutput = conditional(moduleOutput, owner);
    ...
    private static void registerFrame(RecipeOutput output) {
        ...
        ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, TBContent.BED_FRAME_ITEM.get())
                ...
                .save(forResult(output, TBContent.BED_FRAME_ITEM.get()),
                        ResourceLocation.fromNamespaceAndPath("tuttasbeds", "crafting/bed_frame"));
    }
    ...
    private static void registerFrameMattressCombos(RecipeOutput output, MattressFamily family, Mods owner) {
        ...
        registerCombine(output, frame, family.looseMattress().get(), family.bedBare().get(), "frame");

        family.looseMattressCovers().forEach((cover, mattress) -> {
            Supplier<Block> coveredBed = family.bedBasicCovers().get(cover);
            if (coveredBed != null) {
                registerCombine(conditionalForCover(output, cover, owner), frame, mattress.get(), coveredBed.get(), "frame");
            }
        });
    }
    ...
    private static void registerDirectApplyRecipes(RecipeOutput output, MattressFamily family, Mods owner) {
        family.looseMattressCovers().forEach((cover, result) -> {
            registerCombine(conditionalForCover(output, cover, owner), family.looseMattress().get(), DefaultBedIngredients.cover(cover.suffix()), result.get(), "direct");
        });
        family.bedBasicCovers().forEach((cover, result) -> {
            registerCombine(conditionalForCover(output, cover, owner), family.bedBare().get(), DefaultBedIngredients.cover(cover.suffix()), result.get(), "direct");
        });
        ...
        if (mattressId.getNamespace().equals("tuttasbeds")) {
            if (family.material().id().equals("wheat")) {
                ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, mattress)
                        ...
                        .save(forResult(output, mattress), idFor(mattress));
                return;
            }
    ...
    private static void registerCombine(RecipeOutput output, ItemLike base, ItemLike ingredient, Block result, String kind) {
        registerCombine(output, base, Ingredient.of(ingredient), result, kind);
    }

    private static void registerCombine(RecipeOutput output, ItemLike base, Ingredient ingredient, Block result, String kind) {
        ShapelessRecipeBuilder.shapeless(RecipeCategory.DECORATIONS, result)
                .requires(base)
                .requires(ingredient)
                .unlockedBy("has_base", has(base))
                .save(forResult(output, result), idFor(result, kind));
    }
    ...
    private static RecipeOutput forResult(RecipeOutput output, ItemLike result) {
        return output.withConditions(new ItemExistsCondition(BuiltInRegistries.ITEM.getKey(result.asItem())));
    }

    private static RecipeOutput conditional(RecipeOutput output, Mods... mods) {
        ICondition[] conditions = java.util.Arrays.stream(mods)
